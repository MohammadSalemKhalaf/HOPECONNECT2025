...
export const createSponsorship = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to create sponsorship", error: error.message });
  }
};
...
export const getAllSponsorships = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch sponsorships", error: error.message });
  }
};
...
export const getSponsorshipById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch sponsorship", error: error.message });
  }
};
...
export const updateSponsorship = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to update sponsorship", error: error.message });
  }
};
...
export const deleteSponsorship = async (req, res) => {
  try {
    const { id } = req.params;
    const [result] = await pool.query("DELETE FROM sponsorship WHERE id = ?", [
      id
    ]);
    ...
    res
      .status(500)
      .json({ message: "Failed to delete sponsorship", error: error.message });
  }
};
